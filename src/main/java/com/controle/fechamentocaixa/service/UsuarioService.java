...
public class UsuarioService {
   ...
   * @throws ResourceAlreadyExistsException Se já existir um usuário com o email
   *                                        informado
   */
  ...
  public void registrarAcesso(String email) {
    try {
      // Atualiza diretamente o documento bruto para evitar problemas de mapeamento
      Query q = new Query(Criteria.where("email").is(email));
      Update u = new Update().set("dataUltimoAcesso", LocalDateTime.now());
      mongoTemplate.updateFirst(q, u, "usuarios");
    } catch (Exception ex) {
      // Fallback silencioso: evita que falhas de auditoria derrubem o login
      // Log poderia ser adicionado aqui se houver um logger disponível
    }
  }
